...
void COEFICIENTES(double** AE_T,double** BE_T,double** AT_T,double** BT_T,double** CTE_T,double** DTE_T)
{
	...
	//* Para todos los elementos COMIENZA EL PRIMER ELEMENTO (SOBRE EL QUE SE INTEGRA)
		...
		if((tpproE == 1) || (tpproTE == 1)) {
			...
			int arTemp_0;int arTemp_1;int arTemp_2;int arTemp_3;arTemp_2 = 1-1; 
			for(arTemp_0=el*3 - 2;arTemp_0<=el*3;arTemp_0+=1){
            			arTemp_2 += 1;arTemp_3 = 1-1; 
				for(arTemp_1=nd*3 -2;arTemp_1<=nd*3;arTemp_1+=1) {
           			 	arTemp_3 += 1;AE_T[arTemp_0-1][arTemp_1-1] = AE[arTemp_2-1][arTemp_3-1];
            			}
        		}
        
        		int arTemp_4;int arTemp_5;int arTemp_6;int arTemp_7;arTemp_6 = 1-1; 
			for(arTemp_4=el*3 - 2;arTemp_4<=el*3;arTemp_4+=1){
            			arTemp_6 += 1;arTemp_7 = 1-1; 
				for(arTemp_5=nd*3 -2;arTemp_5<=nd*3;arTemp_5+=1) {
		...
		if((tpproT == 1) || (tpproTE == 1)) {
    		...
    		if(tpproTE == 1)     {
        		...
        		for( i=1; i<=3; i++) {
       			 ...
       			 int arTemp_8;int arTemp_9;int arTemp_10;arTemp_10 = 1-1; 
			for(arTemp_8=el*3 - 2;arTemp_8<=el*3;arTemp_8+=1){
           		 	arTemp_10 += 1;
				for(arTemp_9=nd;arTemp_9<=nd;arTemp_9+=1) {
            				CTE_T[arTemp_8-1][arTemp_9-1] = CTE[arTemp_10-1];
            			}
        		}
        
       			int arTemp_11;int arTemp_12;int arTemp_13;arTemp_13 = 1-1; 
			for(arTemp_11=el*3 - 2;arTemp_11<=el*3;arTemp_11+=1) {
            			arTemp_13 += 1;
				for(arTemp_12=nd;arTemp_12<=nd;arTemp_12+=1) {
	...
	shared(tpproTE,tpproT,tpproE,nelT,ndCol,ndT,exT,AE_T,BE_T,AT_T,BT_T,CTE_T,DTE_T,extraux,reg,conT,locT,AE1,BE1,AT1,BT1,C1,D1, intenum,i,j,nd,el,extr,GT,nuT,alT) private(AE,BE,AT,BT,CTE,DTE,DTTE)
        //* Para todos los elementos COMIENZA EL PRIMER ELEMENTO (SOBRE EL QUE SE INTEGRA)
        for( el=1; el<=nelT; el++)	      	
        {
            //* Para todos los nodos COMIENZA EL PRIMER NODO (DESDE EL QUE SE INTEGRA)
